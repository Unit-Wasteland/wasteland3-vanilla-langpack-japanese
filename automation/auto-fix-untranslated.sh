...
2. **各エントリを順次処理**（一括処理禁止）:
   - 各行番号について、以下を実行:
     a) **Read ツールでコンテキスト把握:**
        - 通常: 該当行の前後3-5行を読み込み
        - **短いエントリ（1-5文字）: 前後10-20行を読み込み**（コンテキスト確認必須）
     b) **クォート形式（CRITICAL - CLAUDE.md厳格ルール）:**
        - **クォート数を英語ソースと完全一致させる（追加・削除禁止）**
        - 英語ソースが `string data = " English"` (2個のクォート) → 日本語も `string data = " 日本語"` (2個)
        - 英語ソースが `string data = ""English""` (4個のクォート) → 日本語も `string data = ""日本語""` (4個)
        - **\" エスケープは絶対禁止（構造破壊の原因）**
        - **クォートの追加・削除で数の辻褄を合わせない**
     c) **翻訳判断（コンテキスト重視 - CRITICAL）:**
        - **短いエントリ（1-5文字、短いフレーズ）の場合:**
          * 該当行の前後10-20行を読み込んでコンテキストを確認
          * 会話の流れ、シーンの状況を理解してから翻訳判断
          * 例: "2b.", "13c." などは前後を見ると16進数表現とわかる → 英語のまま
          * 例: "OK", "Yes" などは前後を見てセリフか技術用語か判断
          * **判断できない場合は英語のまま保持**（誤訳防止）
