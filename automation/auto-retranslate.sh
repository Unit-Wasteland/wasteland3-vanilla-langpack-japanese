...
1. **スペイン語参照による翻訳可否判断** (MANDATORY - FIXED 2025-11-05):
   ...
   **判定ロジック**:
   ...
   a) スペイン語が空 ("") の場合:
      - 英語も空 ("") → スキップ（本当に空のエントリ）
      - 英語が非空 → 英語のまま保持（プログラム識別子）

   b) スペイン語が非空の場合:
      - スペイン語が英語と異なる（翻訳されている） → 日本語に翻訳
      - スペイン語が英語と同じ（翻訳されていない） → 以下の優先順で判断:
        1. nouns_glossary.jsonに登録がある → 用語集の訳語で日本語に翻訳（固有名詞）
        2. do_not_translateリストに該当 (Script Node等) → 英語のまま保持
        3. それ以外 → 通常テキストとして日本語に翻訳

   **重要**: 「スペイン語 == 英語」は「プログラム識別子」を意味しない。
            固有名詞（人名、地名など）はスペイン語版でも英語のまま残っていることが多い。
            必ず用語集を確認し、登録があれば日本語に翻訳すること。
