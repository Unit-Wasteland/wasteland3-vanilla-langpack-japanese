...
**⚠️ エスケープ処理は絶対禁止:**
Unity StringTable形式ではバックスラッシュ（`\`）によるエスケープ処理は**一切不要**です。
...
**説明:** バックスラッシュ（`\`）によるエスケープは不要。Unity StringTable形式では `""` （半角ダブルクォート2つ）を直接使用する。
